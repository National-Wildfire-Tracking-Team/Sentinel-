  ...
  return (
    <div
      className="
        ...
        bg-red-50 dark:bg-red-950/90 backdrop-blur-sm
    ...
    >
      ...
      {/* Alert Label */}
      <span className="hidden text-xs font-bold tracking-wider text-red-600 dark:text-red-400 uppercase sm:inline shrink-0">
        Red Flag Warning
      </span>

      <span className="hidden text-red-300 dark:text-red-700 sm:inline">·</span>

      {/* Alert Headline */}
      <button
        onClick={openAlertDetails}
        className="flex-1 text-xs font-medium text-left text-red-800 dark:text-red-200 truncate transition-colors cursor-pointer hover:text-red-950 dark:hover:text-white"
      >
        {current.headline ||
      ...
      {rfwAlerts.length > 1 && (
        <div className="flex items-center gap-1 shrink-0">
          <button
            onClick={previous}
            className="p-0.5 text-red-500 dark:text-red-400 hover:text-red-800 dark:hover:text-red-200 transition-colors"
            aria-label="Previous alert"
          >
            <ChevronLeft size={13} />
          </button>

          <span className="text-xs text-red-500 dark:text-red-400 min-w-[32px] text-center">
      ...
      <button
        onClick={onDismiss}
        className="shrink-0 p-0.5 text-red-500 hover:text-red-800 dark:hover:text-red-200 transition-colors"
