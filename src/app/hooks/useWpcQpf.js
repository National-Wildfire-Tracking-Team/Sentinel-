/**
 * useWpcQpf.js
 * Loads the WPC 24-hour QPF isopleth polygons for a given day.
 * Auto-refreshes every 5 minutes when enabled.
 */

import { useCallback, useEffect, useRef, useState } from 'react';
import { fetchQpfLayer, QPF_LAYER_ID_MAP } from '../api/wpcQpf';
 ...
 */
export function useWpcQpf(enabled = false, activeDay = 'day1') {
  ...
  const load = useCallback(async () => {
    ...
    if (QPF_LAYER_ID_MAP[activeDay] == null) {
      setGeoJSON(EMPTY_FC);
      setValidTime(null);
      return;
    }
    ...
    try {
      setLoading(true);
      setError(null);
      const fc = await fetchQpfLayer(activeDay);
      ...
      if (!mountedRef.current) return;
      setError(err.message || 'Could not load the QPF outlook');
    } finally {
