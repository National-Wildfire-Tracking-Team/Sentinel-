/**
 * useWpcEro.js
 * Loads the WPC Excessive Rainfall Outlook polygons for a given day.
 * Auto-refreshes every 5 minutes when enabled.
 */

import { useCallback, useEffect, useRef, useState } from 'react';
import { fetchEroLayer, ERO_LAYER_ID_MAP } from '../api/wpcEro';
 ...
 */
export function useWpcEro(enabled = false, activeDay = 'day1') {
  ...
  const load = useCallback(async () => {
    ...
    if (ERO_LAYER_ID_MAP[activeDay] == null) {
      setGeoJSON(EMPTY_FC);
      setValidTime(null);
      return;
    }
    ...
    try {
      setLoading(true);
      setError(null);
      const fc = await fetchEroLayer(activeDay);
      ...
      if (!mountedRef.current) return;
      setError(err.message || 'Could not load the excessive rainfall outlook');
    } finally {
